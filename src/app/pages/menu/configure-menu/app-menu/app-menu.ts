...
export class AppMenu {
  ...
  getAppmenu() {
    this.menuService.getAppmenu().subscribe((res) => {
      console.log(res);
  ...
  buildTree(items: AppMenuModel[]): MenuNode[] {
    const map = new Map<number, MenuNode>();
    const roots: MenuNode[] = [];

    // First pass: create nodes
    items.forEach(item => {
      map.set(item.appMenuId, { ...item, children: [], isExpanded: true });
    });

    // Second pass: connect children to parents
    items.forEach(item => {
      // Use appMenuId for map lookup
      const node = map.get(item.appMenuId)!;
  ...
  drop(event: CdkDragDrop<MenuNode[]>) {
    if (event.previousContainer === event.container) {
      ...
      // Root list id is 'root-list', children lists are 'list-{appMenuId}'
  ...
  onSave() {
    ...
    currentFlatList.forEach(item => {
      const original = this.originalMenuItems.find(o => o.appMenuId === item.appMenuId);
      if (original && item.parentId !== original.parentId) {
        ...
        moveAndReorder.push({
          ...
          parentAppMenuId: item.parentId, // Updated key
    ...
    // 2. Identify Reorders
    const movedToParents = new Set(moveAndReorder.map(m => m.parentAppMenuId)); // Updated key reference
    ...
    allParents.forEach(parentId => {
      ...
      const originalChildrenIds = this.originalMenuItems
        .filter(n => n.parentId === parentId)
        .filter(n => currentChildrenIds.includes(n.appMenuId))
        .sort((a, b) => a.sortOrder - b.sortOrder)
        .map(n => n.appMenuId);

      if (JSON.stringify(currentChildrenIds) !== JSON.stringify(originalChildrenIds)) {
        reorders.push({
          parentAppMenuId: parentId,
