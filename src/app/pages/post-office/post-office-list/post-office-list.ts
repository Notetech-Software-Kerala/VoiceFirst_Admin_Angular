...
@Component({
  ...
  templateUrl: './post-office-list.html',
  styleUrl: './post-office-list.css',
})
export class PostOfficeList extends BaseListComponent implements OnInit, OnDestroy {
  ...
  navigatoToDetails(item: PostOfficeModel) {
    const encryptedId = this.encryptionService.encryptForRoute(item.postOfficeId);
    this.router.navigate(['/post-office/details', encryptedId]);
  }
